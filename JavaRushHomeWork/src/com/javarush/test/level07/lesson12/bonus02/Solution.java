...
public class Solution
{
    public static void main(String[] args) throws IOException
    {
        ...
        for (int i = 0; i < list.size(); i++)
        {
            ...
            if (s.length() % 2 == 0)
            {
                for (int j = 1; j <=2; j++) listUpperCase.add(s + " ");
            }
            if (s.length() % 3 == 0 || s.length() == 1)
            {
                for (int j = 1; j <=3; j++) listUpperCase.add(s + " ");
            }
        ...
        {
            String curText = listUpperCase.get(i);
            if (listUpperCase.size() == i+1) System.out.println(curText);
            else if (curText.equals(listUpperCase.get(i + 1)))
            {
                System.out.print(curText);
            }
            else System.out.println(curText);
        }
    }
}
